...
void MainWindow::RenderIntoFrameBuffer(){
	fbo.Bind();
	//mouse control
	float theta = ((float) this->x) / ((float) fbo.size.x) * 360.0f;
	float rho = ((float) this->y) / ((float) fbo.size.y) * 360.0f;

	this->ClearWindow(fbo.size.x, fbo.size.y);
	this->DrawSkybox(fbo.size.x,fbo.size.y);	
	this->DisplayWindowName("FBO Texture 0",fbo.size.x,fbo.size.y);
	glEnable(GL_DEPTH_TEST);
	...
	glm::mat4 modelview_matrix = glm::scale(glm::rotate(glm::rotate(glm::lookAt(
		...
		, theta,glm::vec3(0,1,0))
		, rho, glm::vec3(raxis.x, raxis.y, raxis.z))
		, scaleVec);
...
void MainWindow::UseFramebufferToDrawSomething(){
	//fbo.Bind(1);
	DrawQuadScreen(this->width,this->height,fbo.texture_handles[0]);
	//fbo.Unbind();
}
...
void MainWindow::TakeDown()
{
	...
	if (this->vertex_coordinate_quad_handle != BAD_GL_VALUE)
		glDeleteBuffers(1, &this->vertex_coordinate_quad_handle);
...
bool MainWindow::PostGLInitialize(void * data)
{
	...
	if (!this->vertex_shader.Initialize("tex_shader.vert", "tex_shader.frag"))
		return false;
	...
	glGenBuffers(1, &this->vertex_coordinate_quad_handle);
	assert(this->vertex_coordinate_quad_handle != BAD_GL_VALUE);
	glBindBuffer(GL_ARRAY_BUFFER, this->vertex_coordinate_quad_handle);
	glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(glm::vec3), quadVertexArray, GL_STATIC_DRAW);

	glGenVertexArrays(1, &this->vertex_array_quad_handle);
	glBindVertexArray(this->vertex_array_quad_handle);	
